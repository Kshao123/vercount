...
function parseUrlsFromSiteMap(sitemap) {
	...
	const urls = $('loc').map((index, item) => {
		return $(item).text();
	}).get();
	const filteredUrls = urls.filter(url => url.includes('post')).map((url) => {
		// todo
...
async function updateCountLoop(urls) {
	...
	for (let i = 0; i < urls.length; i++) {
		...
		const getHeaders = (isHtml) => {
			return {
				Referer: isHtml ? `${protocol}//${host}${path}index.html` : `${protocol}//${host}${path}`,
		...
		item.htmlPv = fetchBusuanziData(BUSUANZI_URL, getHeaders(true))
			.then(res => {
				countEntries[key].htmlPv = res.page_pv;
			})
		
		item.rootPagePv = fetchBusuanziData(BUSUANZI_URL, getHeaders())
			.then(res => {
	...
	for (const key in countEntries) {
		...
		if (item.rootPagePv instanceof Promise) {
			try {
				...
				errorPageKeys.push({ key: item.key, reason: error, fields: 'rootPagePv' });
			}
		}
...
async function setRedisKv(data, isOnline) {
	try {
		await fetch(isOnline ? 'https://busuanzi.ksh7.com/redis' : 'http://localhost:3000/redis', {
			method: 'POST',
			headers: {
		...
		console.log(e, 'setRedisKv error');
		process.exit(1);
	}
}
